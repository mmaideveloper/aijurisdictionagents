...
- Speech input can now create a new case from spoken commands in Slovak, English, or German, for example `Create a new case with name ...`, `Vytvor novy pripad s nazvom ...`, `Chcem vytvorit pripad s nazovom ...`, `Vytvor mi novy case s nazvom ...`, or `Erstelle einen neuen Fall mit Namen ...`.
...
- Speech routing now goes through a provider-based speech factory/service layer with `AIJ_SPEECH_MODE=local|azure`. `local` is the default and applies STT to the device/browser runtime when available.
- `AIJ_SPEECHTYPE=message|conversation` controls the interaction flow. `message` is the default: enabling audio plays Jurisdicta's welcome/instruction, the composer microphone fills the input draft, and only the send button, another microphone click, or a spoken send command stops the current dictation. `conversation` preserves the older continuous voice-session flow with silence confirmation.
...
- After the user uploads case documents, the chat inserts one inline upload-status message at that exact point in the conversation and updates it in place to `Documents are uploaded.` or `Document upload failed.`. That status now stays in the transcript as a regular system message.
...
- If the user asks for document status in chat, the app replies locally with the current case document list and each document status. If the user asks for a document analysis/review action while some case documents are still not processed, the app blocks the request locally and explains which documents are still pending.
- After a completed AI discussion session in `AI simulator user` mode, the chat appends a regular system message at the end of the transcript with the latest case validation accuracy, validation summary, legal-data freshness timestamp, and current core model version returned by the API session result metadata. Normal `Real Agent` question/answer turns no longer append validation after every reply.
...
- Default local API base URL for web and desktop targets: `http://127.0.0.1:8080`.
- Local loopback API URLs must use `http`, not `https`; the local Uvicorn API does not serve TLS on port `8080`.
...
```bash
cd mobile_app
flutter test test/profile_voice_update_test.dart
```

For iOS simulator/local device, override `AIJ_API_BASE_URL` with your host IP when `127.0.0.1` does not point at the development machine, for example:
...
Minimal intent mapper example:

```bash
cd mobile_app
dart run examples/intent_mapper_demo.dart
```

Voice compliance regression checks:
...
When voice input is active, the app keeps listening until one of these happens:
- user taps **Send**,
- user says a finish command (`to je vsetko`, `vsetko`, `I am done`, `alles`), or
- the speech service stop flow is triggered manually.

Jurisdicta now also says and shows this finish guidance in the initial listening message.
