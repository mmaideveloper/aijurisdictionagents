...
- Before the first project command or code edit for every implementation task, run `.\scripts\sync_env_profile.ps1 -Mode Pull -Profile codex-agent`. Codex must invoke the script and may inspect its redacted key-name/status output. After a successful pull/audit, Codex may inspect only values explicitly listed in `config/env_profiles.json` under `inspectable_non_secret`, using `scripts/env_config.py inspect`; direct `.env` reads and all secret or non-allowlisted values remain prohibited. If the server is unavailable, run `-Mode Audit -Profile codex-agent -Strict`; continue only with a previously verified local profile. This gate does not apply to read-only review or task preparation.
