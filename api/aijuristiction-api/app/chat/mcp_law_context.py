...
class InternalMcpUnavailableError(RuntimeError):
    """Privacy-safe failure raised after bounded internal MCP retries."""
...
def _build_laws_only_context(
    *,
    ...
    try:
        ...
        return _unavailable_context(
            tool_calls=["searchLaws"], language=language, failure_category=_failure_category(exc)
        )
...
def _call_remote_mcp_tool(
    *,
    ...
    max_attempts = attempts if attempts is not None else _env_int("INTERNAL_MCP_RETRY_ATTEMPTS", 3)
    timeout_seconds = _env_float("INTERNAL_MCP_REQUEST_TIMEOUT_SECONDS", 10.0)
    backoff_seconds = _env_float("INTERNAL_MCP_RETRY_BACKOFF_SECONDS", 1.0)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            _LOGGER.info(
                "internal_mcp_tool_call tool=%s status=ok attempt=%d duration_ms=%d request_id=%s correlation_id=%s",
                name,
                attempt,
            ...
            _LOGGER.warning(
                "internal_mcp_tool_call tool=%s status=retryable_failure attempt=%d duration_ms=%d failure_category=%s request_id=%s correlation_id=%s",
                name,
                attempt,
                int((time.perf_counter() - started) * 1000),
                last_category,
        ...
        raise InternalMcpUnavailableError(category=last_category, attempts=max_attempts)
