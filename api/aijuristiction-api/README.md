...
- Client-facing legal documents, including requests such as `potvrdenie o zaplateni`, now render with a professional JurisDicta document layout: empty header area for now, formal body typography, and a footer with a small JurisDicta logo plus a QR code containing only generation date, API version, core system version, and case ID.
