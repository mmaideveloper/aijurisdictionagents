...
- Admin-selected local defaults and disabled local profiles are preserved across API startup. Use `/app/admin` -> `Modely a ceny` to edit, enable/disable, or set the default free local profile; use `Smerovacie politiky` to edit or disable route policies; use `Lokálne Ollama modely` to import the selected tag, disable linked JurisDigta profiles, or delete the physical runtime model after it is no longer active. Do not disable the current free default until another enabled local profile is selected, otherwise free routing fails closed.
