...
- `MAX_SESSION_CHAT_MESSAGE=10` keeps the latest 10 individual user/assistant messages verbatim in each model request. When the transcript is longer, the API supplies one in-memory summary of older messages before those 10 messages. The complete transcript returned by chat-history endpoints is unchanged. The cache does not survive an API restart; context is rebuilt from whatever session/case messages are restored. Invalid or non-positive values fail explicitly.
- Free/default users use the seeded `local_ollama_default` route: provider `local_ollama`, exact model `qwen3:1.7b`. Local runs default to `http://127.0.0.1:11434/v1`; self-managed Docker production injects the private Docker gateway URL during schema initialization so the API container reaches the host-local Ollama service. Direct free-plan local replies use a compact compliance prompt, strict selected-language guardrails, and capped output to stay inside public edge timeouts on CPU-only Ollama.
- Admin-selected local defaults and disabled local profiles are preserved across API startup. Use `/app/admin` -> `Modely a ceny` to edit, enable/disable, or set the default free local profile; use `Smerovacie politiky` to edit or disable route policies; use `Lokálne Ollama modely` to import the selected tag, remove linked JurisDigta profiles, or delete the physical runtime model after it is no longer active. Not-installed configured-only Ollama rows cannot be set as default and can always be removed from the admin table. Do not disable the current free default until another enabled local profile is selected, otherwise free routing fails closed.
- Provider and model-profile forms accept validated non-secret `model_parameters` JSON. Provider values are defaults and profile values override them; use `null` in a profile to remove an inherited key. For Azure Foundry `gpt-5-mini`, `{"temperature": null}` omits `temperature` entirely and avoids the unsupported `0.2` request. Never place credentials, endpoints, prompts, or case/user data in this field.
...
- Minimal runnable case-export example: `python examples/case_export_demo.py`
...
- Chat query embedding initialization is bounded by `CHAT_EMBEDDING_TIMEOUT_SECONDS` (default `60`); timeout/failure falls back to lexical-only document ranking before LangGraph routing. Streaming chat is bounded by `CHAT_STREAM_TERMINAL_TIMEOUT_SECONDS` (default `660`) and emits a privacy-safe `chat_workflow_timeout` error followed by a failed `done` event instead of waiting indefinitely. See `docs/ISSUE_776_CHAT_WORKFLOW_STARTUP.md`.
