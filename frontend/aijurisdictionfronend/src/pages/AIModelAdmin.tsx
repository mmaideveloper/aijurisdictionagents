            ...
            <section className="admin-panel">
              ...
              {providerMode === "table" ? (
                ...
                <form ref={editFormRef} tabIndex={-1} className="admin-form-stack" onSubmit={(event) => {
                  ...
                  <label>{t("adminProviderType")}<select value={providerForm.provider_type} onChange={(event) => setProviderForm({ ...providerForm, provider_type: event.target.value })}><option value="local">local</option><option value="ollama">ollama</option><option value="azurefoundry">azurefoundry</option><option value="azure_speech">Azure Speech</option><option value="openai">openai</option><option value="openai_compatible">openai_compatible</option></select></label>
            ...
            <section className="admin-panel">
              ...
              {profileMode === "table" ? (
                ...
                <form ref={editFormRef} tabIndex={-1} className="admin-form-stack" onSubmit={(event) => {
                  ...
                  {activeProviders.find((provider) => provider.provider_id === profileForm.provider_id)?.provider_type === "azure_speech" && <button type="button" onClick={() => setProfileForm({ ...profileForm, model_parameters: JSON.stringify({ capability: "speech_to_text", streaming: true, locales: "sk-SK,en-US,de-DE" }) })}>{t("sttConfigureProfile")}</button>}
