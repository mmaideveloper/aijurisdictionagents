...
describe("Sidebar", () => {
  ...
  it("shows compact chat previews and opens the selected case chat", async () => {
    const user = userEvent.setup();
    render(<Sidebar />);

    expect(screen.queryByText(/very long legal conversation/)).toBeNull();
    const chatPreview = screen.getByRole("button", { name: "You: Please continue..." });

    await user.click(chatPreview);

    expect(selectCase).toHaveBeenCalledWith("case-1");
    expect(setCaseCommunicationMode).toHaveBeenCalledWith("case-1", "Chat");
