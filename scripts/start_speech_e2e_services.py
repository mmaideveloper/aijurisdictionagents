"""Run isolated API and MCP services for issue #525 real local acceptance."""
...
API_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:5432/aij_e2e_525_streaming_stt"
LAWS_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:5432/laws_e2e_525_streaming_stt"
...
def main() -> int:
    ...
    # Create/migrate only the fixed synthetic loopback databases; no production storage.
    import psycopg
    ...
    with psycopg.connect("postgresql://postgres:postgres@127.0.0.1:5432/postgres", autocommit=True) as conn:
        for database in ("aij_e2e_525_streaming_stt", "laws_e2e_525_streaming_stt"):
            if not conn.execute("SELECT 1 FROM pg_database WHERE datname=%s", (database,)).fetchone():
                conn.execute(sql.SQL("CREATE DATABASE {}").format(sql.Identifier(database)))
    apply_sql_migrations(project="api", db_option="postgres", target=API_DATABASE_URL, dry_run=False)
    apply_sql_migrations(project="laws", db_option="postgres", target=LAWS_DATABASE_URL, dry_run=False)
    os.environ.update(LAWS_DB_BACKEND="postgres", LAWS_DB_CLOUD=LAWS_DATABASE_URL)
    _seed_synthetic_law()
    ...
    evidence_dir = REPO_ROOT / "runs" / "e2e" / "speech-525-services" / run_id
...
def _required(environment: dict[str, str], name: str) -> str:
    ...
    if value in {"", "unknown-variable"}:
        raise RuntimeError(f"{name} is required for the real issue #525 E2E")
    return value
