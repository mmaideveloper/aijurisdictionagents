"""Run isolated API and MCP services for issue #810 real local acceptance."""
...
API_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:55410/issue810_api"
LAWS_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:55410/issue810_laws"


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--api-port", type=int, default=8190)
    parser.add_argument("--mcp-port", type=int, default=8191)
    ...
    environment.update(
        {
            ...
            "STORE_LOCAL": str(REPO_ROOT / "runs" / "e2e" / "issue-810-legal-explanation" / "files"),
    ...
    evidence_dir = REPO_ROOT / "runs" / "e2e" / "issue-810-legal-explanation" / run_id
    ...
    try:
        for name, target, port, url in (
            ...
            if name == "mcp":
                response = httpx.post(
                    f"{mcp_url}/mcp",
                    headers={"X-JurisDigta-Internal-MCP-Secret": shared_secret},
                    json={"jsonrpc": "2.0", "id": 1, "method": "tools/call",
                          "params": {"name": "searchLaws", "arguments": {
                              "query": "Vysvetli možnosti pre odsúdeného s náramkom, podmienky, môže ísť do práce? Do obchodu?",
                              "country_code": "SK", "limit": 3}}},
                    timeout=60,
                )
                response.raise_for_status()
                assert "issue-810-legal-explanation" in response.text, "Seeded source missing from direct MCP query"
                source = httpx.post(
                    f"{mcp_url}/mcp",
                    headers={"X-JurisDigta-Internal-MCP-Secret": shared_secret},
                    json={"jsonrpc": "2.0", "id": 2, "method": "tools/call",
                          "params": {"name": "getLawText", "arguments": {
                              "document_id": "issue-810-legal-explanation", "max_chars": 3000}}},
                    timeout=60,
                )
                source.raise_for_status()
                assert "EXPLANATION810" in source.text, "Seeded source body missing from MCP"
                (evidence_dir / "direct-mcp.json").write_text(json.dumps({
                    "tools": ["searchLaws", "getLawText"], "sourceId": "issue-810-legal-explanation", "matched": True,
                    "syntheticOnly": True,
                }), encoding="utf-8")
...
def _required(environment: dict[str, str], name: str) -> str:
    ...
    if value in {"", "unknown-variable"}:
        raise RuntimeError(f"{name} is required for the real issue #810 E2E")
    return value
