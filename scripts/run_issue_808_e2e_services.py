"""Run isolated API and MCP services for issue #808 real local acceptance."""
...
API_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:5432/issue_808_e2e"
LAWS_DATABASE_URL = "postgresql://postgres:postgres@127.0.0.1:5433/laws_issue_808_e2e"


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--api-port", type=int, default=8188)
    parser.add_argument("--mcp-port", type=int, default=8178)
    ...
    environment.update(
        {
            ...
            "STORE_LOCAL": str(REPO_ROOT / "runs" / "e2e" / "issue-808-prompt-boundary" / "files"),
    ...
    evidence_dir = REPO_ROOT / "runs" / "e2e" / "issue-808-prompt-boundary" / run_id
...
def _required(environment: dict[str, str], name: str) -> str:
    ...
    if value in {"", "unknown-variable"}:
        raise RuntimeError(f"{name} is required for the real issue #808 E2E")
    return value
