...
$evidenceRoot = Join-Path $repoRoot "runs\e2e\issue-647-prod-mcp-court-decisions\$utcStamp"
...
$env:ISSUE_647_DEPLOYED_COMMIT_SHA = $DeployedCommitSha.ToLowerInvariant()
$env:ISSUE_647_TIMEOUT_MS = [string]$TimeoutMs
$env:ISSUE_647_FINAL_SCREENSHOT_PATH = if ($FinalScreenshotPath) {
    [IO.Path]::GetFullPath((Join-Path $repoRoot $FinalScreenshotPath))
} else {
    ""
}
$env:PW_START_API = "0"
...
try {
    ...
    node node_modules/@playwright/test/cli.js test `
        tests/issue-647-prod-mcp-court-decisions.spec.ts `
        --reporter=list `
        --output=$evidenceRoot
    if ($LASTEXITCODE -ne 0) {
        throw "Issue #647 production E2E failed. Review the sanitized evidence in $evidenceRoot."
    }
} finally {
    Pop-Location
    ...
    Remove-Item Env:ISSUE_647_FINAL_SCREENSHOT_PATH -ErrorAction SilentlyContinue
}

Write-Output "Issue #647 production MCP court-decision E2E passed."
