...
5. Set `COURT_DECISIONS_DB_BACKEND=postgres`, `COURT_DECISIONS_STORAGE_LOCAL=./runs/storage/court-decision-collector/files/sk`, `COURT_DECISIONS_MAX_PDF_BYTES=26214400`, `COURT_DECISION_MCP_SEARCH_TIMEOUT_MS=600000`, and `COURT_DECISIONS_WORKER_POLL_HOURS=1`. Apply all migrations through `0005_background_enrichment_queue.sql`, install/validate the existing local PDF/OCR runtime, and grant write access only to the dedicated storage path. Keep `COURT_DECISIONS_ENRICHMENT_ENABLED=false` until the infrastructure owner approves cycle/rate/retry/disk/retention values from `.env.example`; then validate one bounded cycle and aggregate Prometheus metrics. Roll back by durably pausing enrichment, disabling it, and deploying the previous worker while preserving queue/control rows and PDFs until the approved retention workflow handles them.
