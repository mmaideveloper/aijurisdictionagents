...
- The self-managed deploy script injects `INTERNAL_MCP_BASE_URL=http://jurisdigta-mcp:8070` into the API container so internal assistant law lookups call the dedicated MCP service over the Docker network. Keep the API and MCP on the same private Docker network and provide the same `INTERNAL_MCP_SHARED_SECRET` (or shared `MCP_API_JWT_SECRET`) through the protected server env file. Runtime calls use a 10-second timeout, three attempts, and one-second backoff by default. Deployment now fails closed unless an authenticated metadata-only `getVersion` JSON-RPC call succeeds from inside `jurisdigta-api`; `/health` alone is insufficient. Validate only the safe `status=ready` result, never print the secret or URL. Roll back by restoring the previous image/config together, then run the same internal probe before accepting traffic; do not bypass the gate.
