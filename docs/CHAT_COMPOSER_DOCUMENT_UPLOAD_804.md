...
The status line lists the selected filenames and stays visible through upload, processing, completion, or failure. Pending documents are explicitly described as unavailable for retrieval until processing and indexing finish. A slow import remains pending beyond 30 seconds. Polling stops when the originating component is left. Retrying a failed filename preserves the prior failed record until the replacement upload succeeds, then removes it. Upload errors show a generic localized message rather than raw storage or provider details.
