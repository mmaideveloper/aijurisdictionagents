...
Admin CRUD sections use mutually exclusive list and form states. Selecting a sidebar section shows its grid; selecting Add/Edit hides every grid in that section and focuses the form. Cancel discards the local form state and returns to the grid. A successful save refreshes and returns to the grid, while a failed save preserves the entered form values and displays the error without revealing credentials or other sensitive field values in logs. Switching sidebar sections exits any open form. The synthetic browser regression is runnable with `npm run test:e2e -- e2e/admin-management.spec.ts --grep "admin edit forms replace"` from `frontend/aijurisdictionfronend`; its ignored screenshots under `output/playwright/issue-618/` are retained for review for no more than seven days. The repository-wide minimal example remains `python examples/minimal_demo.py`.
...
The frontend treats the dashboard request as successful only after the backend responds. If the initial request fails, data-backed Admin sections show a loading/error state instead of an empty table, and selecting any such sidebar section retries the authenticated dashboard request. A successful response with zero rows remains a confirmed empty result and is not refetched merely because the administrator changes sections. The Playwright regression scenario is `frontend/aijurisdictionfronend/e2e/issue-617-admin-sidebar-retry.spec.ts`; it uses a synthetic administrator, checks the authenticated retry, and writes transient screenshot and result-manifest evidence under ignored `runs/e2e/issue-617/` for deletion within seven days. The reviewed PR evidence is retained at `docs/screenshots/issue-617/issue-617-admin-users-after-retry.png`.
