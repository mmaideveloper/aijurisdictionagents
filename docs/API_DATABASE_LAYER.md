...
The Admin `Prihlasovacie údaje poskytovateľa` menu edits `ai_model_credentials`, not provider metadata: the form selects an existing provider and stores or rotates a redacted credential secret such as an Azure Foundry/OpenAI `api_key`, while provider Base URL and lifecycle fields remain in `Poskytovatelia`.
