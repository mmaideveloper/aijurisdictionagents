...
- When the Azure ACA job receives `AZURE_MONITOR_ENABLED=true` and `APPLICATIONINSIGHTS_CONNECTION_STRING`, the same worker logs are exported to Application Insights under application name `laws_collector`, which lets the API observability endpoint filter them separately from `api` and `document_processor`. The connection string alone does not enable export.
