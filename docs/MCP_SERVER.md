...
`POST /mcp` remains an HTTP compatibility alias.
...
`POST /mcp` remains accepted for older client records, and `POST /MC` is also
accepted for connector records that were accidentally saved with the truncated
Claude URL `/MC`. OAuth metadata now advertises
`https://mcp.jurisdigta.eu/MCP` as the canonical protected resource. All
JSON-RPC discovery and tool calls require OAuth or an MCP API key.
...
- OAuth discovery URLs and the canonical MCP endpoint
...
- Path-derived authorization server metadata: `GET /.well-known/oauth-authorization-server/mcp`
  and `GET /.well-known/oauth-authorization-server/MCP`
- Dynamic client registration endpoint: `POST /oauth/register`
- Authorization endpoint: `GET /oauth/authorize`
- Token endpoint: `POST /oauth/token`

The OAuth flow uses authorization code with PKCE S256 and returns an
access-token-only response for new connector discovery. Remote clients can use
OAuth Client ID Metadata Documents, dynamic client registration at
`/oauth/register`, or a preconfigured public OAuth Client ID. New dynamic
registrations may return either `200 OK` or `201 Created` with the issued public
client metadata. Use the canonical protected resource value
`https://mcp.jurisdigta.eu/MCP` on the authorization and token requests. The
lowercase `/mcp` endpoint remains an HTTP compatibility alias, but OAuth
metadata, protected-resource discovery, and issued JWT audiences canonicalize to
...
accepted by `POST /MCP` and `POST /mcp`. Token responses include
`Cache-Control: no-store` and `Pragma: no-cache`; they report `scope=mcp:laws`
and do not include a refresh token for new Claude-style connector flows. The
refresh-token grant handler remains for previously issued refresh tokens, but
authorization-server metadata no longer advertises `offline_access` for new
...
Root OAuth discovery and path-specific discovery are both available to Claude,
VS Code, ChatGPT, and other OAuth-capable clients. Protected unauthenticated
`initialize`, `tools/list`, and tool calls return `401` with a
...
Manual JWT generation remains useful for local VS Code setups that pass an `Authorization` header directly. Standards-based remote clients should prefer OAuth discovery. Newly issued OAuth tokens use the canonical `/MCP` protected-resource audience even when the client supplied the lowercase compatibility endpoint.
...
- VS Code: add an HTTP MCP server in MCP settings with the canonical
  `https://mcp.jurisdigta.eu/MCP` endpoint. The lowercase
  `https://mcp.jurisdigta.eu/mcp` endpoint remains a compatibility alias. This minimal `.mcp.json`
...
- `rankLawsByAmendments`: ranks laws published in a selected year by distinct imported amending acts; amendment frequency is disclosed as a proxy, never proof that a law is incorrect.
- `getLawHistory`: returns imported versions and relations for one law document with an explicit incomplete-corpus and human-review warning.
...
- `searchLaws`: searches imported laws by title, identifier, lawyer-facing title, and structured provision text. Natural-language Slovak scenarios with `sort=relevance` are normalized for diacritics, mapped to supported legal concepts, and ranked from actual `law_provisions` text. PostgreSQL provision retrieval builds selective paired prefix queries with a bounded quota per semantic relation (for example purchase/agreement, registration proposal, and parcel/signature formalities), applies a transaction-local planner safeguard that disables sequential scans for this indexed stage, materializes the combined GIN-backed candidate set, and only then joins the latest version effective on or before the database current date plus its metadata; future-effective consolidations are excluded. Relation quotas stop broad purchase matches from crowding property formalities out, and `matched_relation_count` rewards documents supported across independent relations without hard-coding statute identifiers. Primary acts and authoritative concept terms in their titles outrank derivative republications, amending acts, and incidental provision matches. Such results include `matched_provisions`, `relevant_sections`, compact `relevant_section_ranges` (which may bridge up to two repealed or unmatched neighboring sections), `retrieval_basis`, and a confidence indicator. Exact identifiers remain metadata-first. The response always requires human legal review and supports `published_year`, `year_filter_mode=published_in`, `sort=relevance|latest`, `limit`, `offset`, and `include_summaries`. A broad newest-laws request with `sort=latest` searches the whole selected jurisdiction, while a topical newest query keeps its topic filter. Requested summaries are derived only from imported public metadata and marked `summary_status=metadata_derived`. `sort=latest` orders by latest law publication/effective metadata.
- `getLawText`: returns bounded latest imported text for a law document id. For large codes, pass `section_number` or `section_start`/`section_end` to retrieve structured provision text; add `paragraph_number` when one paragraph of one section is required. Structured anchors are used before the legacy flattened-artifact fallback. Use `offset` and `max_chars` when pagination is needed.
- `searchCourtDecisions`: searches the dedicated court-decision store across metadata, ready pseudonymized summaries, enriched public text, and content chunks. It returns court/date/ECLI/file-number metadata by default; use `include_snippets=true` for pseudonymized excerpts and `include_summaries=true` for available pseudonymized summaries. Conversational Slovak queries are reduced to legal-topic terms, and a phrase such as `posledných 5` supplies the default limit when no explicit `limit` is passed. Use `court_name` for a specific court (for example `Okresny sud Poprad`); `court_type` remains the generic category filter. Court names are matched exactly after case, whitespace, punctuation, and diacritic normalization. Use `sort=latest` to order by the normalized calendar issue date; invalid or missing dates sort last and are identified by `issue_date_status` and the response `data_quality` object. `date_desc`, `newest`, `newest_first`, and `latest_first` are accepted as aliases. Coverage metadata states the newest indexed issue date and enriched count; “latest” always means latest available in the JurisDigta corpus, not guaranteed complete national coverage. MCP court-decision search is bounded by a server-side PostgreSQL connect timeout and scoped statement timeout (`COURT_DECISION_MCP_SEARCH_TIMEOUT_MS`, default `600000`) so slow database calls return a structured degraded payload instead of hanging. Logs record filter presence, query length, limit, duration, error kind, request ID, and correlation ID, but not the raw query, credentials, tokens, snippets, summaries, or court-decision text.
...
- `getLegalSearchResult`: fetches the async legal search result. Results are scoped to the authenticated user and expire after `MCP_ASYNC_SEARCH_RETENTION_SECONDS`.
...
For an issue-based prompt, the client can call `searchLaws` directly, for example `{"query": "chcem kupno predajnu zmluvu na zahradu", "country_code": "SK", "sort": "relevance"}`. The server searches provision text and returns grounded snippets and section numbers. A client should cite only the returned provisions, obtain focused text with `getLawText`, disclose that the result is source discovery rather than legal advice, and keep a human reviewer in the loop for document preparation.

The production-scale regression smoke query is `pripare kupno predoajnu zmluvu`. Its misspellings still select the purchase-contract concept. A qualified purchase-agreement phrase also searches property-transfer formalities so document-preparation clients receive the Civil Code and cadastral sources requested by the regression contract; a bare ambiguous query such as `predaj` does not activate that expansion. The result remains source discovery, not a claim that every returned source applies to the user's facts.

To validate local code against a configured production-style PostgreSQL laws database, run `python examples/issue_636_prod_laws_smoke.py --expect-law "40/1964 Zb."`. Add repeated `--expect-law` arguments for a scenario-specific regression contract, for example `162/1995 Z. z.` for a land-transfer query. The opt-in script does not print the connection value or provision bodies. It verifies the planned index path and writes a minimized JSON/HTML report under ignored `runs/issue-636/`; retain the evidence for no more than seven days.
...
- Redacted from wire logs: authorization and cookie headers, MCP API keys,
  ...
  identity-card fields, raw legal queries, law document ids, MCP content text,
  and returned snippets. The MCP envelope and result byte counts remain visible.
