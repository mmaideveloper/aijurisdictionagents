...
start_court_decision_collector() {
  ...
  docker run -d \
    ...
    -e COURT_DECISIONS_SOURCE_TIMEOUT_SECONDS="${COURT_DECISIONS_SOURCE_TIMEOUT_SECONDS:-90}" \
    -e COURT_DECISIONS_SOURCE_RETRY_ATTEMPTS="${COURT_DECISIONS_SOURCE_RETRY_ATTEMPTS:-3}" \
    -e COURT_DECISIONS_SOURCE_RETRY_BACKOFF_SECONDS="${COURT_DECISIONS_SOURCE_RETRY_BACKOFF_SECONDS:-5}" \
    ...
    -e COURT_DECISIONS_DAILY_NEW_LIMIT="${COURT_DECISIONS_DAILY_NEW_LIMIT:-10000}" \
    -e COURT_DECISIONS_DISCOVERY_OVERLAP_PAGES="${COURT_DECISIONS_DISCOVERY_OVERLAP_PAGES:-2}" \
    -e COURT_DECISIONS_BACKFILL_PAGES_PER_CYCLE="${COURT_DECISIONS_BACKFILL_PAGES_PER_CYCLE:-10}" \
    -v "$DEPLOY_ROOT/runs:/workspace/runs" \
    aijuristiction-api:local \
